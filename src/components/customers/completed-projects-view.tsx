...
export default function CompletedProjectsView() {
  const { data: response, isLoading } = useCustomersProjectsListQuery('completed');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">List of successfully completed projects in the last 30 days.</p>
        </div>
      </div>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  projects.map((item, index) => {
                    ...
                    return (
                      <tr key={item.leadId} className="border-b border-[#E5E7EB] text-[13px] text-slate-700 last:border-b-0 hover:slate-50">
                        ...
                        <td className="px-4 py-4 text-slate-700">{item.revenue ? `$${item.revenue.toLocaleString()}` : "$0"}</td>
