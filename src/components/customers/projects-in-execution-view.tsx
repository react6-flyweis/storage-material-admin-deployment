...
export default function ProjectsInExecutionView() {
  const { data: response, isLoading } = useCustomersProjectsListQuery('active');
  const projects = response?.data?.projects || [];
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Projects in Execution</h2>
          <p className="text-sm text-gray-500">List of all active projects currently being processed.</p>
        </div>
      </div>
            ...
            <table className="min-w-[900px] w-full border-collapse text-left">
              <thead>
                <tr className="border-b border-[#E5E7EB] bg-gray-50 text-[12px] font-medium text-slate-500 uppercase">
                  ...
                  <th className="px-4 py-3 font-medium text-slate-500">Assigned Plant</th>
                  ...
                  projects.map((item, index) => {
                    ...
                    return (
                      <tr key={item.leadId} className="border-b border-[#E5E7EB] text-[13px] text-slate-700 last:border-b-0 hover:bg-slate-50">
                        ...
                        <td className="px-4 py-4 text-slate-700">{item.assignedPlant || "N/A"}</td>
