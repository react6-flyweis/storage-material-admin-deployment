...
export default function LeadDocumentsTab({ leadId }: Props) {
  ...
  const engineeringFiles = documents.filter((doc: any) => doc.type === "engineering");
  const drawings = documents.filter((doc: any) => doc.type === "drawing");
  const photos = documents.filter((doc: any) => doc.type === "photo");
  const otherFiles = documents.filter((doc: any) => !["engineering", "drawing", "photo"].includes(doc.type));

  const getExt = (name: string) => name.split('.').pop()?.toUpperCase() || 'FILE';

  const renderDocumentCard = (doc: any, iconType: "file" | "image" = "file") => (
    <div key={doc._id} className="border rounded-lg p-4 relative pt-6 flex flex-col justify-between bg-white">
      <div className="absolute -top-3 right-4">
        <Badge className="bg-green-100 text-green-700 hover:bg-green-100 font-medium capitalize border-0">
          Uploaded
        </Badge>
      </div>
        ...
        <div className="space-y-10">
          ...
          {engineeringFiles.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {engineeringFiles.map((doc: any) => renderDocumentCard(doc))}
              </div>
            </div>
          )}
          ...
          {drawings.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {drawings.map((doc: any) => renderDocumentCard(doc))}
              </div>
            </div>
          )}
          ...
          {photos.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {photos.map((doc: any) => renderDocumentCard(doc, "image"))}
              </div>
            </div>
          )}
          ...
          {otherFiles.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {otherFiles.map((doc: any) => renderDocumentCard(doc))}
              </div>
            </div>
          )}
        </div>
