...
const LEAD_DETAILS_TABS = [
  ...
  { value: "follow_ups", label: "Follow-Up History" },
...
export default function LeadDetails() {
  ...
  const updateLifecycleMutation = useMutation({
    mutationFn: (status: string) =>
      updateLeadLifecycleProvider(leadId!, { lifecycleStatus: status }),
    onSuccess: () => {
  ...
  const currentStepId =
    leadData?.lifecycle?.status || lead?.lifecycleStatus || "initial_contact";
  let currentStepIndex = SALES_LIFECYCLE_STAGES.findIndex(
  ...
  const progressPercentage = Math.max(
    ...
    Math.min(
      100,
      (currentStepIndex / Math.max(1, SALES_LIFECYCLE_STAGES.length - 1)) * 100,
    ),
  );
      ...
      <Card className="flex-1">
          ...
          >
            RFQ
                ...
                <AssignSalesDialog
                  leadId={leadId!}
                  currentSalesId={
                    typeof lead?.assignedSales === "object"
                      ? lead?.assignedSales?._id
                      : lead?.assignedSales
                  }
                  trigger={
                    <Button
                ...
                {leadId && (
                  <DeleteLeadDialog
                    ...
                    trigger={
                      <Button
                        variant="outline"
                        className="bg-white hover:bg-red-50 hover:text-red-600 border-red-200 text-red-600 h-9 px-4 text-sm font-normal rounded-md"
                      >
        ...
        <CardContent>
          <Tabs
            ...
            className="w-full"
          >
            <TabsList className="w-full flex flex-wrap justify-between bg-transparent gap-3 mb-5">
              {LEAD_DETAILS_TABS.map((tab) => (
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="flex-1 py-3 text-sm border-gray-400 bg-gray-100 text-gray-500  data-[state=active]:border-black "
                >
                  {tab.label}
                </TabsTrigger>
              ))}
                    ...
                    <div className="bg-gray-50/50 rounded-lg p-4 flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900">
                          Assigned to:{" "}
                          {typeof lead?.assignedSales === "object"
                            ? (lead?.assignedSales as any)?.name
                            : typeof lead?.assignedSales === "string"
                              ? lead?.assignedSales
                              : "Unassigned"}
                        </p>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5">
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-[#f8fafc] rounded-lg border border-[#f1f5f9]">
                    ...
                    <span className="text-sm font-bold text-gray-900">
                      {lead?.length ? `${lead.length} ft` : "N/A"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between px-4 py-3 bg-[#f8fafc] rounded-lg border border-[#f1f5f9]">
                    ...
                    <span className="text-sm font-bold text-gray-900">
                      {lead?.height ? `${lead.height} ft` : "N/A"}
                    </span>
                  </div>
                  ...
                  <div className="relative flex justify-between">
                    {SALES_LIFECYCLE_STAGES.map((step, index) => {
                      ...
                      return (
                        ...
                        >
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center font-semibold text-sm relative z-10 ${circleClass}`}
                          >
                            {index + 1}
                          </div>
                          <div className="mt-3 text-center">
                            <p
                              className={`text-xs font-semibold ${isPending ? "text-gray-400" : "text-gray-900"} whitespace-pre-line`}
                            >
                              {step.label}
                            </p>
                            <p
                              className={`text-[10px] mt-1 whitespace-pre-line ${isPending ? "text-gray-400" : "text-gray-500"}`}
                            >
                              {statusText}
                            </p>
                          </div>
                ...
                <div className="bg-gray-50/50 border rounded-xl p-5 mb-6 grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="pr-6 border-r border-gray-200 border-dashed">
                    <h4 className="text-sm font-semibold text-gray-900">
                      Progress ({currentStepIndex + 1} of{" "}
                      {SALES_LIFECYCLE_STAGES.length})
                    </h4>
                    <p className="text-sm text-gray-500 mt-2 capitalize">
                      Current step: {currentStepId.replace(/_/g, " ")}
                    </p>
                  </div>
            ...
            <p className="text-sm text-gray-500 mt-2 leading-relaxed flex-1">
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                      ...
                      <div className="py-4 space-y-4">
                        <div className="space-y-2">
                          ...
                          <Select
                            onValueChange={(val) =>
                              updateLifecycleMutation.mutate(val)
                            }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white rounded-xl border shadow-sm p-6 flex flex-col items-center">
                  <h3 className="text-lg font-bold text-[#0f172a] self-start mb-6">
                    Project Status
                  </h3>
                  <div className="relative w-40 h-40 mb-6">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <span className="text-xs text-gray-500 mt-1">
                        of {SALES_LIFECYCLE_STAGES.length}
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-white rounded-xl border shadow-sm p-6 flex flex-col h-full max-h-[400px]">
                  <h3 className="text-lg font-bold text-[#0f172a] mb-6 border-b pb-4 shrink-0">
                    Notes History
                  </h3>
                  <div className="space-y-4 text-sm text-gray-500 overflow-y-auto flex-1 pr-2">
                    {leadData?.leadNotes && leadData.leadNotes.length > 0 ? (
                      leadData.leadNotes.map((noteItem: any) => (
                        <div
                          key={noteItem._id}
                          className="bg-gray-50 p-4 rounded-lg border border-gray-100 relative"
                        >
                  ...
                  <div className="space-y-6">
                    {Object.keys(scoreBreakdown).map((key) => {
                      ...
                      const percentage = Math.min(
                        100,
                        Math.max(0, (item.points / maxPoints) * 100),
                      );
            ...
            <TabsContent value="call_history" className="mt-6">
              <LeadCallHistoryTab
                lead={{
                  id: lead?.jobId || leadId || "",
                  name: customer?.firstName || lead?.name || "Unknown",
                }}
              />
            </TabsContent>
