          ...
          >
            RFQ
                ...
                {leadId && (
                  <DeleteLeadDialog
                    leadId={leadId}
                    leadName={customer?.firstName ? `${customer.firstName} ${customer.lastName || ''}`.trim() : lead?.projectName}
                    onSuccess={() => navigate('/leads')}
                    trigger={
                      <Button variant="outline" className="bg-white hover:bg-red-50 hover:text-red-600 border-red-200 text-red-600 h-9 px-4 text-sm font-normal rounded-md">
            ...
            <p className="text-sm text-gray-400 mt-1">{lead?.jobId || leadId || 'PRO-053'}</p>
          </div>
